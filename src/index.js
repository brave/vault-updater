...
mq.setup((sender) => {
  ...
  db.setup(sender, (mongo) => {
    // read the releases from a database
    pgc.connectToURL(config.database_url, (err, pg) => {
      ...
      releasesAccess.readReleasesFromDatabase((err, releases) => {
        ...
        // Routes
        server.route([common.root].concat(releaseRoutes, extensionRoutes, crashes, monitoring, androidRoutes, iosRoutes, apiRoutes))
